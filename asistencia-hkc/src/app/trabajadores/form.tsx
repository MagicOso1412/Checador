...
/** Alta y edición de trabajadores en un solo formulario (modo según si viene `id` en la ruta). */
export default function TrabajadorFormScreen() {
  const { id } = useLocalSearchParams<{ id?: string }>();
  const esEdicion = Boolean(id);

  const { obtenerPorId, crearTrabajador, actualizarTrabajador, eliminarTrabajador, guardando } =
    useTrabajadoresAdminStore();
  ...
  useEffect(() => {
    if (!id) return;
    (async () => {
      const trabajador = await obtenerPorId(id);
      if (trabajador) {
        setNumeroEmpleado(trabajador.numeroEmpleado);
        setNombre(trabajador.nombre);
        setApellidoPaterno(trabajador.apellidoPaterno);
        setApellidoMaterno(trabajador.apellidoMaterno);
  ...
  const handleEliminar = () => {
    ...
    Alert.alert(
      ...
      "El trabajador dejará de aparecer como activo, pero su historial de asistencias se conserva. ¿Continuar?",
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              setError(err instanceof Error ? err.message : "No se pudo dar de baja al trabajador");
            }
          },
        },
      ],
  ...
  return (
    <View className="flex-1 bg-background">
      <ScreenHeader
        title={esEdicion ? "Editar Trabajador" : "Nuevo Trabajador"}
        onBack={() => router.back()}
      />
      ...
      <ScrollView className="flex-1 px-4 pt-4" contentContainerStyle={{ paddingBottom: 24, gap: 16 }}>
        <View className="gap-3 rounded-2xl border border-border bg-card p-4" style={shadowSm}>
          <Field label="Número de empleado" value={numeroEmpleado} onChangeText={setNumeroEmpleado} />
        ...
        {esEdicion ? (
          ...
          >
            <Text className="text-sm font-medium text-foreground">
              {activo ? "Trabajador activo" : "Trabajador inactivo"}
            </Text>
